        ...
        if route:
            try:
                ...
                if route.name == 'static':
                    ...
                    self.request.path_info = self.request.path_info.replace(
                        self.static_path, ''
                    )
                ...
                request_exception_signal.send(
                    self, request=self.request, exc_info=exc_info
                )
                handler_500 = self.handler500(
